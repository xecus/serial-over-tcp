...
class TestSerialToNetworkBridge(unittest.TestCase):
    ...
    def test_setup_serial_success(self, mock_serial):
        ...
        mock_serial.assert_called_once_with(
            ...
            parity=serial.PARITY_NONE,  # Mapped from 'N'
            stopbits=serial.STOPBITS_ONE,  # Mapped from 1
...
class TestCommandLineArguments(unittest.TestCase):
    ...
    def test_valid_arguments_parsing(self):
        ...
        with mock.patch('sys.argv', ['serial_tcp_server.py', '/dev/ttyUSB0', '9999']):
            ...
            parser.add_argument('-d', '--databits', type=int, default=8, choices=[5, 6, 7, 8],
                help='Data bits')
            parser.add_argument('-p', '--parity', default='N', choices=['N', 'E', 'O'],
                help='Parity')
            parser.add_argument('-s', '--stopbits', type=int, default=1, choices=[1, 2],
                help='Stop bits')
    ...
    def test_custom_arguments_parsing(self):
        ...
        args = (
            parser.parse_args(
                ['/dev/ttyUSB1', '8888', '-b', '115200', '-d', '7', '-p', 'E', '-s', '2', '-v']))
