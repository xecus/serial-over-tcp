...
class VirtualSerialDevice:
                ...
                try:
                    ...
                    if os.path.exists(self.device_path) or os.path.islink(self.device_path):
                        if os.path.islink(self.device_path):
                            ...
                            self.logger.error(f"Path exists but is not a symlink: {self.device_path}")
                            return False
    ...
    def close(self):
        ...
        if self.device_path and (os.path.exists(self.device_path) or os.path.islink(self.device_path)):
            try:
                os.unlink(self.device_path)
                        ...
                        if self.virtual_device and self.virtual_device.master_fd is not None:
                            try:
                                ...
                                if bytes_written != len(data):
                                    self.logger.warning(f"Partial write to virtual device: {bytes_written}/{len(data)} bytes")
                                safe_data = data[:50] if len(data) > 50 else data
                                self.logger.debug(f"TCP -> Virtual: {safe_data}{'...' if len(data) > 50 else ''}")
                            except OSError as e:
                            ...
                            if data:
                                ...
                                if self.tcp_socket:
                                    try:
                                        ...
                                        safe_data = data[:50] if len(data) > 50 else data
                                        self.logger.debug(f"Virtual -> TCP: {safe_data}{'...' if len(data) > 50 else ''}")
                                    except socket.error as e:
