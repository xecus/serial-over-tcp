...
class SerialToNetworkBridge:
        ...
        try:
            ...
            with self.clients_lock:
                if len(self.clients) >= self.max_clients:
                    msg = (f"Connection limit reached ({self.max_clients}), "
            ...
            try:
                welcome_msg = (f"Connected to {self.serial_port} at "
                               f"{self.serial_baudrate} baud\r\n")
                client_socket.send(welcome_msg.encode())
            except (socket.error, OSError) as e:
                msg = f"Failed to send welcome message to {client_address}: {e}"
                self.logger.warning(msg)
                return
                ...
                try:
                    ...
                    if ready[0]:
                        try:
                            ...
                            # Validate data size
                            if len(data) > 4096:
                                msg = (f"Large data packet ({len(data)} bytes) "
                            ...
                            if self.serial_conn and self.serial_conn.is_open:
                                try:
                                    ...
                                    self.logger.debug(f"Sent to serial: {safe_data}{suffix}")
    ...
    def _handle_client_with_cleanup(self, client_socket, client_address):
        """Wrapper for handle_client that ensures thread cleanup"""
        try:
            self.handle_client(client_socket, client_address)
        finally:
            # Remove this thread from the list when it finishes
            current_thread = threading.current_thread()
            try:
                self.client_threads.remove(current_thread)
            except ValueError:
                pass  # Thread may have already been removed during cleanup

    def _cleanup_finished_threads(self):
        """Remove finished threads from client_threads list"""
                    ...
                    if self.serial_conn.in_waiting > 0:
                        try:
                            ...
                            if data:
                                safe_data = data[:50] if len(data) > 50 else data
                                suffix = '...' if len(data) > 50 else ''
                                self.logger.debug(f"Received from serial: {safe_data}{suffix}")

                                # Send to all connected clients (thread-safe)
                                disconnected_clients = []
                                with self.clients_lock:
                                    # Work with copy to avoid lock contention
                                ...
                                for client in clients_copy:
                                    try:
                                        ...
                                        msg = f"Unexpected error sending to client: {e}"
